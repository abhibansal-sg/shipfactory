...
from headframe.telemetry import append_jsonl, hello_shakedown, on_claim, parse_usage
...
def test_jsonl_and_claim(tmp_path, monkeypatch):
    ...
    rows = [json.loads(line) for line in (tmp_path / "headframe" / "telemetry.jsonl").read_text().splitlines()]
    assert rows[0] == {"event": "custom", "n": 1}
    assert rows[1]["task_id"] == "T1" and rows[1]["run_id"] == 9


def test_hello_shakedown():
    assert hello_shakedown() == "headframe-live"
