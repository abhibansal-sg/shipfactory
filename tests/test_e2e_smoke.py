...
from shipfactory import policy, store
from shipfactory.spawn import _RUNNING, shipfactory_spawn, reap_finished


def _write_seats(home: Path) -> None:
    (home / "profiles" / "dev").mkdir(parents=True)
    (home / "profiles" / "verifier").mkdir(parents=True)
    factory = home / "shipfactory"
    factory.mkdir()
...
def _run_row(home: Path, task_id: str) -> sqlite3.Row:
    conn = sqlite3.connect(home / "shipfactory" / "shipfactory.db")
...
def test_real_dispatch_spawn_reap_and_policy(monkeypatch, tmp_path):
    ...
    try:
        ...
        dispatched = kanban_db.dispatch_once(conn, spawn_fn=shipfactory_spawn, board="e2e")
        ...
        kanban_db.dispatch_once(conn, spawn_fn=shipfactory_spawn, board="e2e")
        outcome = _reap_one()
