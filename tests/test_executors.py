...
def test_codex_extract_text_finds_sentinel_in_jsonl():
    ...
    assert codex.extract_text("no json here\nSHIPFACTORY_RESULT: done x") == "no json here\nSHIPFACTORY_RESULT: done x"
...
def test_claude_extract_text_finds_sentinel_in_stream_json():
    ...
    log = "\n".join([
        ...
        '{"type":"result","result":"SHIPFACTORY_RESULT: done shipped the fix"}',
    ])
    text = claude.extract_text(log)
    assert text.splitlines()[-1] == "SHIPFACTORY_RESULT: done shipped the fix"
...
def test_worktree_git_root_helper(tmp_path):
    from shipfactory.executors.base import worktree_git_root
