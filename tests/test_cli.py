...
def test_init_writes_skeleton_and_initializes_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("headframe.store.init_db", lambda: called.append(True))
    cli.main(["init"])
    text = (tmp_path / "headframe" / "seats.yaml").read_text()
...
def test_pause_and_resume_use_store_contract(monkeypatch):
    calls = []
    monkeypatch.setattr("headframe.store.set_seat_paused", lambda seat, paused: calls.append((seat, paused)))
...
def test_costs_parses_since_duration(monkeypatch):
    calls = []
    monkeypatch.setattr("headframe.store.costs_rollup", lambda by, days: calls.append((by, days)) or [])
...
def test_policy_set_uses_frozen_signature(monkeypatch):
    calls = []
    monkeypatch.setattr("headframe.store.set_policy", lambda task, policy: calls.append((task, policy)))
...
def test_verdict_argument_order(monkeypatch):
    calls = []
    monkeypatch.setattr("headframe.policy.record_verdict", lambda *args: calls.append(args) or {"ok": True})
...
def test_sync_delegates_explicitly(monkeypatch):
    calls = []
    monkeypatch.setattr("headframe.github_sync.sync", lambda **kw: calls.append(kw) or {})
