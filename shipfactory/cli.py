...
def register_cli(parser: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("name"); p.add_argument("--profile", required=True); p.add_argument("--executor", required=True, choices=("hermes", "codex", "claude", "grok")); p.add_argument("--model", required=True); p.add_argument("--reasoning", default="medium"); p.add_argument("--role", required=True); p.add_argument("--max-concurrent", type=int, default=1); p.add_argument("--provider"); p.add_argument("--base-url"); p.add_argument("--provider-model")
    p = _handler(verbs, "seat-update", "update a Factory employment contract", _seat_update)
    p.add_argument("name"); p.add_argument("--profile"); p.add_argument("--executor", choices=("hermes", "codex", "claude", "grok")); p.add_argument("--model"); p.add_argument("--reasoning"); p.add_argument("--role"); p.add_argument("--max-concurrent", type=int); p.add_argument("--provider"); p.add_argument("--base-url"); p.add_argument("--provider-model")
