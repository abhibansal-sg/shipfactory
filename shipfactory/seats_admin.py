...
from shipfactory.config import EXECUTORS, FactoryConfigError
...
def _seats_path() -> Path:
    return _hermes_home() / "shipfactory" / "seats.yaml"
...
def _write_seat(name: str, values: dict[str, Any], *, create: bool, provider_config: dict[str, Any] | None) -> dict[str, Any]:
    path = _seats_path()
    document, header = _load_mapping(path, missing={"company": "shipfactory", "seats": {}})
