...
def activate(conn: Any, instance: dict[str, Any], recipe: dict[str, Any], step_def: dict[str, Any], step: dict[str, Any], parameters: dict[str, Any], parents: list[str]) -> str | None:
    ...
    if primitive in {"agent_task", "review_gate"}:
        return kanban_db.create_task(conn, title=title, body=body, assignee=params["seat"], workspace_kind=params["workspace"], parents=parents, idempotency_key=key, max_runtime_seconds=int(params.get("max_runtime_seconds", 1800)), max_retries=int(params.get("max_retries", 2)))
