...
def _board(board: str | None) -> str:
    ...
    if board:
        return board
    return str(getattr(_module("factory.config").load_seats(), "company", ""))
...
def _conflict_log(winner: str, loser: str, issue: dict[str, Any], task: dict[str, Any]) -> None:
    ...
    if not home:
        return
    path = Path(home) / "factory" / "sync-conflicts.jsonl"
...
def _github_closed_is_allowed(task_id: str) -> bool:
    ...
    return bool(_module("factory.policy").policy_satisfied(task_id))
