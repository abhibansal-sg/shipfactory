...
into ~/.hermes/plugins/shipfactory/).
...
GitHub Issues <--sync--> kanban board (per company) <-- factory daemon
                                                          |  dispatch_once(spawn_fn=shipfactory_spawn)
                                                          v
...
factory/
  ...
  cli.py                # `hermes shipfactory <verb>` argparse tree
  executors/
...
`$HERMES_HOME/shipfactory/seats.yaml`:
```yaml
...
`shipfactory_spawn(task, workspace, board) -> Optional[int]`:
...
3. executor=codex|claude -> Executor.build_cmd():
   ...
   - Spawn subprocess.Popen (fire-and-forget, new session), redirect output
     to $HERMES_HOME/shipfactory/runs/<task_id>-<ts>.log, return pid.
...
5. On child exit (daemon reaps via pid polling — same pattern as kanban's
   ...
   `SHIPFACTORY_RESULT: done|blocked <one-line summary>`).
...
- ENFORCEMENT (two hooks, zero core changes):
  a. kanban_task_completed hook: when a worker completes a task that has
     ...
     passed.
  b. Stage verdicts are recorded via `hermes shipfactory verdict <task> \
...
- JSONL mirror at $HERMES_HOME/shipfactory/telemetry.jsonl (append-only).
- Rollups (pure SQL): by seat/day, by executor, by task; `hermes shipfactory
...
- One-way-each-direction explicit sync (NOT a hidden daemon thread):
  `hermes shipfactory sync [--board X] [--repo owner/name]` +
...
- org chart: `hermes shipfactory org` prints the tree ASCII.
...
plugins/shipfactory/dashboard/{manifest.json,plugin_api.py,dist/} —
the same mechanism the kanban plugin tab uses. The standalone server code
may be deleted on sight after the tab ships; hermes shipfactory dashboard CLI
...
`hermes shipfactory <verb>`: init (write seats.yaml skeleton + db), seats,
...
- Tools (service-gated on factory.db existing): shipfactory_verdict,
  shipfactory_costs, shipfactory_monitor_add — JSON-string returns per Hermes tool
  contract.
...
- A tests/test_e2e_smoke.py: init -> create task on a tmp board (real
  kanban_db API against tmp db) -> shipfactory_spawn with executor=hermes
...
spawn.py MUST export:
  shipfactory_spawn(task, workspace: str, *, board=None) -> int|None   # signature matches kanban spawn_fn
...
Sentinel (executor prompts MUST instruct, daemon MUST parse):
  last line of harness output: `SHIPFACTORY_RESULT: done|blocked <summary>`
...
- test_e2e_smoke.py: NO stubs of factory modules. tmp HERMES_HOME; create a
  ...
  ~/Developer/products/hermes-mobile); real shipfactory.store init_db; real
  seats.yaml via factory.config; add a task; run kanban dispatch_once with
  the REAL shipfactory_spawn but executor command swapped to a /bin/sh stub
  harness that prints tokens + SHIPFACTORY_RESULT sentinel (spawn a REAL
...
- test_adversarial.py, minimum:
  ...
  d. Malformed SHIPFACTORY_RESULT lines (garbage, multiple, mid-log) — parser
...
2. `review_gate`
   ...
   `SHIPFACTORY_VERDICT: {"outcome":"approve","body":"..."}`

   or:

   `SHIPFACTORY_VERDICT: {"outcome":"request_changes","target_step":"step-id","body":"..."}`
